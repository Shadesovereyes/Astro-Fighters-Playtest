...
> **Direction-convention correction (2026-08-27):** this historical QA was authored using the reference-sheet/source-slot labels. Astro Fighters canonical gameplay uses `S = front` and `N = back`. Interpret direction names below through `KAIRO_PACKAGE_07_DIRECTION_CONVENTION.md`. In particular, source slot `NW` corresponds to canonical `SW`; rear source slots `SE/S/SW` correspond to canonical `NE/N/NW`; source clean-chest slots `N/NE/E/W` correspond to canonical `S/SE/E/W`.
...
These labels are source-slot terminology. The later canonical direction convention generalizes this into the full eight-direction source-slot mapping.
...
Rear **source slots** (`SE`, `S`, `SW`) suppress the inner-top overlay; in canonical gameplay those facings are `NE`, `N`, `NW`.
...
The dense lower rows are extracted by row-wide connected components and object-center assignment instead of overlapping rectangles. The remaining gourd source-slot `NW` clipping and shoulder-tie neighboring fragments from v23 were specifically corrected in v24.
...
The four genuine source gaps remain in source slot `NW`, which is canonical gameplay `SW`:
...
1. a true source-quality clean-chest canonical `S / SE / E / W` body master exists;
2. a true hairless body master exists;
3. clothing scale and sleeve/cuff/hand seams are manually corrected in all eight canonical directions;
...
6. the four missing canonical `SW` equipment views are authored;
